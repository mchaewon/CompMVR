...
def get_image_paths(dataroot):
    return _get_paths_from_images(dataroot, True)


 
def _extract_last_int(s: str) -> Optional[int]:
    """
    rect_001_3_r5000.png  → 1    
    rect_049_3_r5000.png  → 49
    frame_001.png         → 1    
    frame_0001_QP37.png   → 1
 
    """
    stem = os.path.splitext(os.path.basename(s))[0]
 
    m = re.match(r'rect_(\d+)', stem)
    if m:
...
def _natural_file_sort_key(path: str):
    name = os.path.basename(path)
    num = _extract_last_int(name)
    if num is None:
        return (1, name.lower())
    return (0, num, name.lower())

def _list_sorted_files(dir_path: str, file_type: str):
    ...
    for fname in os.listdir(dir_path):
        ...
        if not os.path.isfile(p):
            continue
        if file_type == 'image' and is_image_file(fname):
            files.append(p)
 
    files.sort(key=_natural_file_sort_key)
    return files

 
def _normalize_codec_name(codec_name: str) -> str:
    return {"H264": "AVC"}.get(codec_name, codec_name)
 
 
def _parse_codec_scene_qp_from_flat_dir(dirname: str):
    m = re.match(r'^(?P<codec>[^_]+)_(?P<scene>.+)_QP(?P<qp>\d+)$', dirname)
    if not m:
        return None
    return {
        "codec": _normalize_codec_name(m.group('codec')),
        "scene": m.group('scene'),
        "qp":    f"QP{m.group('qp')}",
    }
 
 
def _get_paths_from_images(path: str, return_meta: bool = False):

    path = os.path.abspath(path)
    images = []
 
    pattern = os.path.join(path, "*")
    sub_dirs = [d for d in glob.glob(pattern) if os.path.isdir(d)]
 
    for sub_dir in sub_dirs:
        folder_name = os.path.basename(sub_dir)
 
        parsed = _parse_codec_scene_qp_from_flat_dir(folder_name)
 
        if parsed:
            codec     = parsed["codec"]
            scene     = parsed["scene"]
            folder_qp = parsed["qp"]
            target_dir = os.path.join(sub_dir, "frame")
        else:
            codec      = "RAW"
            scene      = folder_name
            folder_qp  = None
        ...
        files = _list_sorted_files(target_dir, 'image')
 
        for p in files:
            fname     = os.path.basename(p)
            qp_match  = re.search(r'_QP(\d+)', fname)
            frame_qp  = f"QP{qp_match.group(1)}" if qp_match else folder_qp
 
            frame_num = _extract_last_int(p)
 
            if frame_num is None:
                print(f"[WARN] view number extract fail: {p}")
                continue
